...
const UserManagement = () => {
  ...
  const [respondHandlerActiveModalState, setRespondHandlerActiveModalState] = useState({ state: false, userId: "" });
          ...
          <div className="table_section employee_table">
            ...
            {user?.map((userDetails, index) => (
              <div className="table_data" key={index}>
                ...
                <div className="col_20p">
                  <h6>
                    {userDetails?.locationId?.map((ele) => (ele.name))?.toString()}
                  </h6>
                  ...
                  <h6 className='action_wrraper'>
                    ...
                    <div className="form-check form-switch " title="enable/disable" >
                      <input
                        ...
                        type="checkbox"
                        checked={checkedStatus[userDetails._id] || false}
                        id={`toggleSwitch${userDetails._id}`}
                        // onChange={(event) =>
                        //     changeUserStatus(event, userDetails._id, userDetails.isActive)
                        // }
                        onChange={() => setRespondHandlerActiveModalState({ state: true, userId: userDetails?._id })}
