...
const CategoryList = () => {
  ...
  //get Category list on initial Load
  async function getAllCategory(page, searchString, isSearch = false) {
    try {
      setLoader(true);
      const serverResponse = await communication.getAllCategory(page, searchString);
      if (serverResponse?.data?.status === "SUCCESS") {
        ...
        toast(serverResponse?.data?.message);
        setCategoryList([]);
  ...
  const deletecategory = async () => {
    if (selectedCheckboxes.length > 0) {
      ...
      toast.info("Please select which category you want to delete");
    }
  };
