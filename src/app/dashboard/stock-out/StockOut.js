...
const StockOut = () => {
    ...
    // Fetch purchase order list on initial load
    const getPOList = async (page, searchString) => {
        try {
            setLoader(true);
            const serverResponse = await communication.getStockOutMaterial({ page, searchString });
            if (serverResponse?.data?.status === "SUCCESS") {
                setPoList(serverResponse?.data?.po);
    ...
    async function deletePo(poId) {
        try {
            ...
            const serverResponse = await communication.deleteCompletedPo(poId);
            if (serverResponse?.data?.status === "SUCCESS") {
    ...
    useEffect(() => {
        getPOList(currentPage, searchString);
    }, [isPageUpdated]);
        ...
        <>
...

            {
                ...
                <CustomResponseHandlerModal
                    ...
                    successHandler={() => { deletePo(modalState?.poId) }}
                    cancelHandler={() => { setModalStates((prev) => ({ ...prev, deletePo: false, poId: "" })) }}
                />
            }

            {modalState.viewPO && <ViewPo data={modalState?.data} setModalStates={setModalStates} />
            }
                                ...
                                return <div className="table_data" key={index}>
                                    ...
                                    <div className="col_30p">
                                        <h6 className='action_wrraper'>
                                            <div title="view" onClick={() => setModalStates(pre => ({ ...pre, viewPO: true, data: data }))}>
                                            ...
                                            <div title='View Bill' onClick={() => setModalStates(pre => ({ ...pre, viewPdf: true, data: data }))}>
                                                <FontAwesomeIcon icon={faFileInvoice} />
                                            </div>
                                            <div title="delete" onClick={() => setModalStates((prev) => ({ ...prev, deletePo: true, poId: data._id }))}>
