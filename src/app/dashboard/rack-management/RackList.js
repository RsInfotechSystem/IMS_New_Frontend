...
const RackList = () => {
  ...
  async function getRackList(page, searchString, isSearch = false) {
    try {
      setLoader(true);
      const serverResponse = await communication.getRackList(page, searchString);
  ...
  const handleCheckboxChange = (e) => {
    ...
    setSelectAllChecked(
      !selectedCheckboxes.includes(checkboxId) && selectedCheckboxes.length + 1 === rackList?.length
    );
  ...
  const deleteRole = async () => {
    if (selectedCheckboxes.length > 0) {
      ...
      toast.info("Please select which rack you want to delete");
    }
  };
    ...
    <>
      ...
      {isEnableDisable.modal && (
        <CustomResponseHandlerModal
          status="warning"
          message={`Do you want to ${isEnableDisable.action} rack?`}
          successHandler={() => {
