...
const CreatePo = () => {
  ...
  const totalSum = updatedProducts
    ...
    ?.reduce(
      (acc, modelData) =>
        acc +
        (getValues("type") === "WHOLESALE"
          ? (modelData?.wholeSalePrice ?? modelData.price) * modelData.quantity
          : (modelData?.retailPrice ?? modelData.price) * modelData?.quantity),
      0
    );
  ...
  return (
    <>
      ...
      {modalState.viewPdf && (
        <ViewBill pdfData={modalState?.data} setModalStates={setModalStates} />
      )}
      {/* top header  */}
      <div className="top_header">
        <div className="tab_title">
          {[undefined, null, ""]?.includes(params.get("poId"))
        ...
        <div className="form_list_layout_wrapper my-4">
                  ...
                  return (
                    <div className="table_data" key={index}>
                      ...
                      <div className="col_20p">
                        <h6>
                          {getValues("type") === "WHOLESALE"
                            ? (data?.wholeSalePrice ?? data?.price) * data?.quantity
                            : (data?.retailPrice ?? data?.price) * data?.quantity}
                        </h6>
                      </div>
          ...
          <div className="d-flex align-items-start justify-content-start gap-4 py-3">
            <div className="fw-medium text-secondary">
              ...
              <InputBox type={"text"} value={totalSum} disable={true} />
            </div>
        ...
        <div className="d-flex align-items-center justify-content-center gap-3 my-3">
          ...
          {params.get("type") !== "approval" && (
            <Button
              ...
              name={[undefined, null, ""]?.includes(params.get("poId")) ? "Create" : "Update"}
              className="button"
            />
          )}
          {params.get("type") === "approval" && modalState.viewPrintBill === false && (
            <Button
              type="submit"
              onClick={handleSubmit(receivePaymentPo)}
              name={"Receive Payment"}
              className="button"
            />
          )}
          {params.get("type") === "approval" && modalState.viewPrintBill && (
            <Button onClick={() => printBill()} name={"Print Bill"} className="button" />
          )}
          <Button
            type="submit"
            onClick={() => router.push("/dashboard/po-management")}
            name={"Cancel"}
            className="button"
          />
        </div>
