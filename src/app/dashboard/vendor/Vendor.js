...
const Vendor = () => {
    ...
    async function getUserList(page, searchString, isSearch = false) {
        try {
            setLoader(true);
            const serverResponse = await communication.getVendorList(page, searchString);
            if (serverResponse?.data?.status === "SUCCESS") {
                setUser(serverResponse?.data.vendor);
                ...
                serverResponse?.data?.vendor?.forEach((vendor) => {
                    initialCheckedStatus[vendor._id] = vendor.isActive || false;
                });
    ...
    const handleDelete = async (userIds) => {
        try {
            setLoader(true);
            let payload = { vendorIds: [...userIds] };
            let response = await communication.deleteVendors(payload);
    ...
    const changeUserStatus = async (vendorId) => {
        try {
            setLoader(true);
            let response = await communication.changeVendorStatus({ vendorId });
            if (response?.data?.status === "SUCCESS") {
                toast.success(response.data.message, {
                    autoClose: 1500, // 1.5 seconds
                });
    ...
    return (
        <>
            ...
            <div className="top_header">
                <div className="tab_title">Vendor Management</div>
                            ...
                            <>
                                {user?.map((userDetails, index) => (
                                    <div className="table_data" key={index}>
                                        ...
                                        <div className="col_15p">
                                            <h6>{userDetails?.code ?? "--"}</h6>
                                        </div>
                                        <div className="col_30p">
                                            <h6>{userDetails?.name}</h6>
                                        </div>
                                        <div className="col_35p">
                                            <h6 style={{ textTransform: "lowercase" }}>{userDetails?.email ? userDetails?.email : "--"}</h6>
                                        </div>
                                        <div className="col_20p">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={() =>
                                                            setRespondHandlerActiveModalState({
                                                                state: true,
                                                                action: userDetails?.isActive ? "disable" : "enable",
