...
const EngineerReportList = () => {
    ...
    const getReportMaterialList = async (page = 1, searchString = "") => {
        try {
            setLoader(true);
            const serverResponse = await communication.getEngineerWiseStockList(
                searchParams.get("userId"),
                page,
                searchString
            );
            if (serverResponse?.data?.status === "SUCCESS") {
                const flattenedRecords = serverResponse?.data?.materials
                                ...
                                {reportDetails?.map((data, index) => {
                                    return (
                                        <div className="table_data" key={data?._id}>
                                            ...
                                            <div className="col_20p">
                                                <h6>{data?.stockId?.categoryId?.name ?? "--"}</h6>
                                            </div>
                                            <div className="col_20p">
                                                <h6>{data?.stockId?.brandId?.name ?? "--"}</h6>
                                            </div>
                                            <div className="col_20p">
                                                <h6>{data?.stockId?.modelId?.name ?? "--"}</h6>
                                            </div>
                                            <div className="col_30p">
                                                <h6>{data?.stockId?.locationId?.name ?? "--"}</h6>
                                            </div>
                                            <div className="col_25p">
                                                <h6>{data?.stockId?.conditionType ?? "--"}</h6>
                                            </div>
                                            <div className="col_20p">
                                                <h6>{data?.stockId?.quantity ?? "--"}</h6>
                                            </div>
                                            <div className="col_20p">
                                                <h6>{data?.stockId?.status ?? "--"}</h6>
                                            </div>
                                        </div>
                                    );
                                })}
...
export default EngineerReportList;
