...
const EngineerReport = () => {
    ...
    const [modalState, setModalStates] = useState({ data: [], isOpen: false })

    const getReportMaterialList = async () => {
        try {
            setLoader(true);
            const serverResponse = await communication.getEngineerAssignedReport();
    ...
    return (
        <>
            ...
            {modalState.isOpen && <ViewAssignedMaterial setModalStates={setModalStates} data={modalState.data} />}

            <div className="top_header">
                <div className="tab_title" style={{ textTransform: "capitalize" }}>
                    {"Enginner Assigned Material Report"}
                </div>
            </div>
                            ...
                            return (
                                <>
                                    <div className="table_data">
                                        ...
                                        <div className="col_30p">
                                            <h6>{data?.assignQuantity}</h6>
                                        </div>

                                        <div className="col_25p">
                                            <h6 onClick={() => setModalStates({ isOpen: true, data: data.material })}> <CustomBtn name={"View Material"} />  </h6>
                                        </div>
                                    </div>
                                </>
                            );
