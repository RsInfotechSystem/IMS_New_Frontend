...
function TopHeader() {
    ...
    const getNotificationCount = async (controller) => {
        try {
            const serverResponse = await communication.fetchNotificationCount(
                controller
            );
        ...
        <div className="top_header_wrapper">
            <div className="top_header_nav_wrapper">
                {
                    userDetails?.designationName === "admin" ?
                        adminNav.map((tabDetail, index) => (
                            <h6 onClick={() => navigate(tabDetail?.url)} key={index} style={{ color: currentUrl[2] === tabDetail?.activeUrl ? "#198754" : "#000000" }}>{tabDetail?.tab}</h6>
                        ))
                        :
                        topHeaderNavArray?.filter((ele) => userDetails?.modules?.includes(ele?.tab))?.map((tabDetail, index) => (
                            <h6 onClick={() => navigate(tabDetail?.url)} key={index} style={{ color: currentUrl[2] === tabDetail?.activeUrl ? "#198754" : "#000000" }}>{tabDetail?.tab}</h6>
                        ))
                }
            </div>
            <div className="user_profile_wrapper">
                <div className="notification_icon_wrapper">
                    <FontAwesomeIcon icon={faBell} className="notification_icon fontAwesome_icon" onClick={() => { router.push("/panel/admin/notification") }} />
                    {
                        notificationCount > 0 && <div className="notification_count_wrapper">
                ...
                <div className="user_profile">
                    ...
                    <div className="profile_detail position-relative cursor_pointer"
