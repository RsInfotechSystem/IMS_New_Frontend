...
import { accountTabsArray } from "@/utilities/accountTabsArray";
...
function AccountTabs() {
  ...
  const accountAccesableTabs = userDetails?.tabAccess?.find((ele) => ele?.module === "Account")?.tabAccess
  // Extract the tab names 
  const tabNames = accountAccesableTabs?.map(item => item?.tabName);
  // Filter the accountTabsArray to include only those tabs that match the tab names from tabNames
  const filteredTabs = accountTabsArray?.filter(tab => tabNames?.includes(tab?.tab));

  // Ensure that the Dashboard tab is always included
  const dashboardTab = accountTabsArray.find(tab => tab.tab === "Dashboard");
